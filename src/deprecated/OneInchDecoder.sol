...
library OneInchDecoder {
    ...
    function _updateToTokenSwapParams(
        IBaseSwapProxy.SwapParams calldata _toTokenSwapParams,
        uint256 updatedAmount,
        uint256 updatedValue
    ) internal returns (IBaseSwapProxy.SwapParams memory) {
        // Decoded and updated amount
        bytes4 sig = abiDecodeSig(_toTokenSwapParams.data);

        // if (sig == bytes4(keccak256("unoswa)))

        (address srcToken, uint256 amount, uint256 minReturn, bytes32[] memory pools) = abi.decode(
            _toTokenSwapParams.data[4:],
            (address, uint256, uint256, bytes32[])
        );

        bytes memory data = abi.encodeWithSelector(sig, srcToken, updatedAmount, minReturn, pools);

        return IBaseSwapProxy.SwapParams(_toTokenSwapParams.to, updatedAmount, updatedValue, data);
    }

    // https://ethereum.stackexchange.com/questions/48576/solidity-assembly-question-mstore
    function updateData(bytes memory _data, uint256 _amount, uint256 _minReturn) public view returns (bytes memory) {
