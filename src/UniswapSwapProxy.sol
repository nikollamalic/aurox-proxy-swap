...
contract UniswapSwapProxy is IUniswapSwapProxy, AccessControlEnumerable, Pausable, ReentrancyGuard, BaseSwapProxy {
    ...
    function proxySwapWithFee(
        ...
        require(!_isEth(_fromToken), "Swapping from ETH not supported");
        ...
        (uint256 feeTotalInETH, uint256 feeTotalInFromToken) = _calculatePercentageFeeInETH(
            _fromToken,
            _amount,
            _gasRefund
        );
        ...
        if (!_isEth(_fromToken) && !_isEth(_toToken) && feeTotalInETH > 0) {
            ...
            (uint256 amountUsedForFee, ) = uniswapV2Router._swapTokensForExactETH(
                _fromToken,
                feeTotalInETH,
                feeTotalInFromToken,
                address(this)
        ...
        uint256 scaledAmount = _scaleAmountFromTokenDecimals(_toToken, _amount, _getDecimals(_fromToken));

        uint256 toTokenExchangeRate = _getExchangeRate(_fromToken, _toToken);
    ...
    function calculatePercentageFeeInETH(
        ...
        uint256 _amount,
        uint256 _gasRefund
    ) external view override returns (uint256 feeTotalInETH, uint256 feeTotalInFromToken) {}
